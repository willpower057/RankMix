...
class FRMIL(nn.Module):
    def __init__(self, args):
        ...
        self.cls_token = nn.Parameter(torch.Tensor(1, 1, self.dim_hidden))
        nn.init.xavier_uniform_(self.cls_token)
        
        self.conv_head = torch.nn.Conv2d(self.dim_hidden, self.dim_hidden, 3, 1, 3//2,  groups=self.dim_hidden)  
        torch.nn.init.xavier_uniform_(self.conv_head.weight)
        
        self.selt_att  = MAB(self.dim_hidden, self.dim_hidden, num_heads)
        self.fc   = nn.Sequential(
            nn.Linear(self.dim_hidden, self.num_outputs),
        )
        
        self.mode = 0
        self.distilled_score_head = None
        self.distilled_bag_head = None
        
    def add_distilled_bag_head(self):
        self.distilled_bag_head = nn.Sequential(
            nn.Linear(self.dim_hidden, self.num_outputs),
        )

    def add_distilled_score_head(self):
        self.distilled_score_head = nn.Sequential(
            nn.Linear(self.dim_hidden, 1),
            nn.Sigmoid()
        )
