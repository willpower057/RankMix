...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        ...
        mag_idx = 0
        print("self._target_levels", self._target_levels);print("target_levels", target_levels);print("mag_list", mag_list)
...
class DeepZoomStaticTiler(object):
    ...
    def _run_image(self, associated=None):
        ...
        print("first_level", first_level, "MAG_BASE", MAG_BASE, "self._base_mag", self._base_mag)
...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg', img_class=None):
    print('\n Organizing patches')
    img_name = img_slide.split(os.sep)[-1].split('.')[0]
    img_class = img_slide.split(os.sep)[2] if img_class is None else str(img_class)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--dataset_txt', nargs='+', type=str, default=None, help='txt file for dataset address')
    parser.add_argument('--class_tsv', type=str, default=None, help='Dataset directory')
    parser.add_argument('-d', '--dataset', type=str, default='NCKU', help='Dataset name')
    ...
    path_base = os.path.join(dataset_dir, args.dataset)
    if len(levels) == 2:
        out_base = os.path.join(dataset_dir, args.dataset, 'pyramid')
    else:
        out_base = os.path.join(dataset_dir, args.dataset, 'single')
    print("dataset_path:", path_base)
    all_slides = glob.glob(os.path.join(path_base, '*.'+args.slide_format)) \
        + glob.glob(os.path.join(path_base, '*/*.'+args.slide_format)) \
        + glob.glob(os.path.join(path_base, '*/*/*.'+args.slide_format))      
    
             
    if args.class_tsv is not None:
        df = pd.read_csv(os.path.join(path_base, args.class_tsv), sep='\t')
    
    
    for idx, c_slide in enumerate(all_slides):        
        print('Process slide {}/{}: {}'.format(idx+1, len(all_slides), c_slide))
        start = time.time();print("levels", levels)        
        if args.class_tsv is not None:            
            img_class = df.loc[df['imagename'] == c_slide.split('/')[-1]+'_merged.png'].iloc[0,2]
            print(f"img_class: {img_class}")
        elif args.dataset.startswith("C16_dataset"):
            img_class = c_slide.split(os.sep)[3]
            print(f"img_class: {img_class}")
        else:
            img_class = None        
        ...
        nested_patches(c_slide, out_base, levels, ext=args.format, img_class=img_class)
