...
def convert_rankmix_dataset(dataset='rankmix_c16'):
    if dataset=='rankmix_c16':
        dataset_pth = 'datasets/C16_dataset_c16_low99_v0'    
        for token in ['training/normal', 'training/tumor', 'testing/images']:
            os.makedirs(f'{dataset_pth}/{token}-npy', exist_ok=True)
            pths = glob.glob(f'{dataset_pth}/{token}/*.csv')
            for pth in tqdm(pths, desc=f'converting {args.dataset} csv to npy, {token}'):
                df = pd.read_csv(pth)
                feats = df.to_numpy()
                np.save(pth.replace(token, f'{token}-npy').replace('.csv', '.npy'), feats)

def split_rankmix_dataset(dataset='rankmix_c16'):
    if dataset =='rankmix_c16':
        dataset_name = 'C16_dataset_c16_low99_v0'
    else:
        dataset_name = dataset

    os.makedirs(f'datasets/{dataset_name}/remix_processed', exist_ok=True)
    train_list_txt = open(f'datasets/{dataset_name}/remix_processed/train_list.txt', 'w')
    test_list_txt = open(f'datasets/{dataset_name}/remix_processed/test_list.txt', 'w')
    train_labels, test_labels = [], []    
    
    if dataset=='rankmix_c16':
        train_bags_csv = 'datasets/C16_dataset_c16_low99_v0/training/training.csv' 
        test_bags_csv = 'datasets/C16_dataset_c16_low99_v0/testing/testing.csv'
        train_bags_path = pd.read_csv(train_bags_csv)
        test_bags_path = pd.read_csv(test_bags_csv)
        reference_csv = pd.read_csv(
                os.path.join('..', 'dataset', 'C16_dataset', 'testing', 'reference.csv'),
        ...
        test_bags_path = test_bags_path.sort_values(by=['0'], ignore_index=True)
        test_bags_path['label'] = reference_csv['label']        
        
        for i in tqdm(range(len(train_bags_path)), desc='splitting Camelyon16/training'):
            feats_npy_pth = train_bags_path.iloc[i].iloc[0]
            feats_npy_pth = os.path.join(feats_npy_pth.replace('normal', 'normal-npy', 1))
            feats_npy_pth = os.path.join(feats_npy_pth.replace('tumor', 'tumor-npy', 1))
            feats_npy_pth = os.path.join('..', 'dsmil-wsi', feats_npy_pth)
            feats_npy_pth = feats_npy_pth.replace('.csv', '.npy')        
            train_list_txt.write(f'{feats_npy_pth},{train_bags_path.iloc[i].iloc[1]}\n')
            train_labels.append(train_bags_path.iloc[i].iloc[1])        

        for i in tqdm(range(len(test_bags_path)), desc='splitting Camelyon16/testing'):
            feats_npy_pth = test_bags_path.iloc[i].iloc[0]        
            feats_npy_pth = os.path.join('..', 'dsmil-wsi', feats_npy_pth.replace('images', 'images-npy', 1))
            feats_npy_pth = feats_npy_pth.replace('.csv', '.npy')
            test_list_txt.write(f'{feats_npy_pth},{test_bags_path.iloc[i].iloc[1]}\n')
            test_labels.append(test_bags_path.iloc[i].iloc[1])       
           
    train_labels = np.array(train_labels)
    test_labels = np.array(test_labels)
    np.save(f'datasets/{dataset_name}/remix_processed/test_bag_labels.npy', test_labels)
    np.save(f'datasets/{dataset_name}/remix_processed/train_bag_labels.npy', train_labels)
    print('done')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='Camelyon16', choices=[
        'Camelyon16', 'Unitopatho', 'rankmix_c16',
    ])
    ...
    elif args.dataset.startswith('rankmix') or args.dataset.startswith('imb'): 
        if args.task[0] == 'convert':
            convert_rankmix_dataset(args.dataset)
            args.task.pop(0)
            
        if args.task[0] == 'split':
            split_rankmix_dataset(args.dataset)
            args.task.pop(0)
